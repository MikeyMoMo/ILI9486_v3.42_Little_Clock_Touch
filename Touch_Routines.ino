...
/****showSelectedClock******************************************************/
{
  ...
  if (initScreen) getXchangeRate();  // Update each 3 hours on the top of the hour.
  ...
  if (whichToShow == dtz_Show) {
    if (initScreen) {  // Hourly init
      ...
      ofr.setCursor(10, 3);           ofr.printf(topCity.c_str());     // Top city name
      ofr.setCursor(10, yCenter + 3); ofr.printf(bottomCity.c_str()); // Bottom city name
      ...
      if (pktValidity) {  // Good data from the web site? Print results.
        ...
        ofr.setCursor(tft.width(), yCenter + 3); ofr.rprintf(myBuildString.c_str());  // Bottom city name
      }
    ...
    if (initScreen) {
      ...
      ofr.setCursor(dispWidth / 2, 0); ofr.cprintf(myBuildString.c_str());  // Header for Binary Clock
    ...
    if (initScreen) {
      tft.fillScreen(DarkerRed);
      initAnalog(homeTime.hour(), homeTime.minute(), homeTime.second());
    } else {
...
/***************************************************************************/
{
  ...
  WhenIsPayDay(homeTime.month(), homeTime.day(), homeTime.year());
...
/***************************************************************************/
{
  ...
  bool gotHit = false;  // Set to true if day found, else leave false for recurse.
  yPos = -5;  // Baseline anchor position and Line 1 position.
  int line1 = yPos, line2 = yPos + 80, line3 = yPos + 160, line4 = yPos + 240;
  ...
  if (SS_Pay_Wed_DOM == 0) {
    ...
    //    switch (SS_Pay_Wed_DOM) {  // Make it English-like a little bit
    //      case 1: case 21: case 31: sLine2 += "st"; break;
    //      case 2: case 22:          sLine2 += "nd"; break;
    //      case 3: case 23:          sLine2 += "rd"; break;
    //      default:                  sLine2 += "th"; break;
    //    }
    ...
    if (d == SS_Pay_Wed_DOM) {  //Is this a direct hit on payday?
      ...
      int nowJulian = calcJulian(homeTime.year(), homeTime.month(), homeTime.day());
      ...
      if ((pdJulian - nowJulian) > 1) {  // If more than 1 day till payday
        ...
        sLine4 = "from now.";
      } else {  // We have taken care of a direct hit so all that is left is "tomorrow".
        gotHit = true;
        sLine3 = "tomorrow";
      }
    }
...
/****ShowCalendar***********************************************************/
{
  ...
  // Draw the grid lines for the calendar dates.  Horizontal first, then vertical.
  y = 0;
  ...
  for (y = 0; y < 6; y++) {
    if (y > 1)
      // 4 will be shorter then the rest to open up the space for the month and year.
  ...
  Serial.printf("First of the month for month %i is %i\r\n", showMonth, calDOW);
  Serial.printf("Dates for: showMonth %i, showYear %i, showMonthLength %i\r\n",
                showMonth, showYear, showMonthLength);
...
/***************************************************************************/
{
  ...
  for (event = 0; event < CalHighlightCt; event++) {
    ...
    if ((HLEvents[event].DOM >= homeTime.day() && HLEvents[event].DOM < 100)
        && (HLEvents[event].Month == homeTime.month() || HLEvents[event].Month == 0)) {
      tft.setTextColor(ILI9341_YELLOW, DarkerRed);  // Foreground color, Background color
  ...
  if (!foundSome) {
    tft.setTextDatum(TL_DATUM);  // This stuff makes no sense to me.
    tft.setTextColor(ILI9341_YELLOW, DarkerRed);  // Foreground color, Background color
    tft.drawString("No events found.", 0, 0);
  }
  unsigned int startMS = millis();
