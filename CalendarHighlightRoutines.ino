...
bool HighlightToday(int Month, int DOM, int Year, int &RectColor, int &TextColor) {
  ...
  for (HLStruct = 0; HLStruct < CalHighlightCt; HLStruct++) {
    if (DOM == HLEvents[HLStruct].DOM) {
      ...
      if ((HLEvents[HLStruct].Month == Month) || (HLEvents[HLStruct].Month == 0)) {
        ...
        if  ((Month == homeTime.month()) && (homeTime.day() <= DOM) || (Month != homeTime.month())) {
          if (homeTime.day() == DOM)  // If the event is today, override the user text color only.
  ...
  if ((homeTime.day() == DOM) && (Month == homeTime.month())) { // Don't highlight today's day in another month.
