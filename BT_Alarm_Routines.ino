...
/***************************************************************************/
{
  ...
  switch (cUserReq)
  {
    ...
    case 'H': case '?':
      //      if (whichInput == BTin) {
      //        SerialBT.println(); //delay(BTxmitWait);
      //        SerialBT.print(homeTime.dateTime()); //delay(BTxmitWait);
      //        SerialBT.printf(" vers %.2f\r\n", Vers); delay(BTxmitWait);
      //      }
      Serial.println(); Serial.print(homeTime.dateTime());
      Serial.printf(" vers %.2f\r\n", Vers);
      for (int temp = 0; temp < helpLines; temp++) {
        //        if (whichInput == BTin) {
        //          SerialBT.println(helpString[temp]);
        //          // Something in the BT chain cannot keep up with out this.
        //          //  The last lines are missing without the delays.
        //          delay(BTxmitWait);  // So delay to let everyone get caught up.
        //        }
...
/***************************************************************************/
{
  ...
  int i, myRand;
  randomSeed(homeTime.hour() + homeTime.minute() + homeTime.second() + millis());
